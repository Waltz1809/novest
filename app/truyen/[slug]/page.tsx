...
export default async function NovelDetailPage({ params }: PageProps) {
    ...
    const isCollaborator = session?.user?.id ? novelBasic.collaborators.some(c => c.userId === session.user.id) : false;
    const canSeeDrafts = isAdmin || isUploader || isCollaborator;
                                    ...
                                    <div className="flex flex-col gap-3 text-left">
                                        ...
                                        <div className="flex flex-wrap gap-3 pt-2">
                                            {firstChapter ? (
                                                ...
                                                <button disabled className="flex items-center gap-2 px-6 py-2.5 bg-gray-100 text-muted-foreground font-bold rounded-lg cursor-not-allowed">
                                                    <BookOpen className="w-5 h-5" />
                                            ...
                                            <LibraryButton
                                                ...
                                                className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-white text-foreground border border-gray-200 hover:border-amber-500 font-medium transition-all"
                                            />
                                            ...
                                            {/* Quick Edit Button - For uploader or collaborator */}
                                            {(isUploader || isCollaborator) && (
                                                <Link
                                                ...
                                                <div className="flex flex-col justify-center min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-1 text-xs text-muted-foreground flex-wrap">
                                                        ...
                                                        {related.genres.slice(0, 2).map((genre, idx) => (
                                                            <span key={`desktop-${idx}`} className="px-1.5 py-0.5 rounded-full bg-gray-100 text-[10px] text-emerald-600 hidden lg:inline">
                                                                {genre.name}
                                                            </span>
                                                        ))}
                                                        {related.genres.slice(0, 3).map((genre, idx) => (
                                                            <span key={`mobile-${idx}`} className="px-1.5 py-0.5 rounded-full bg-gray-100 text-[10px] text-emerald-600 lg:hidden">
                                                                {genre.name}
                                                            </span>
                                                        ))}
                                                    </div>
